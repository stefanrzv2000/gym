...
class CarRacing(gym.Env, EzPickle):
    ...
    def render_road(self):
        ...
        vl.draw(gl.GL_QUADS)
        if self.last_v1 is not None:
            if self.verbose: print("deleting_g")
            self.last_v1.delete()
        self.last_v1 = vl
    ...
    def render_indicators(self, W, H):
        ...
        vertical_ind(7, 0.01 * self.car.wheels[0].omega, (0.0, 1, 1))  # ABS sensors
        vertical_ind(8, 0.01 * self.car.wheels[1].omega, (0.0, 1, 1))
        vertical_ind(9, 0.01 * self.car.wheels[2].omega, (0.2, 1, 1))
        vertical_ind(10, 0.01 * self.car.wheels[3].omega, (0.2, 1, 1))
        horiz_ind(20, -10.0 * self.car.wheels[0].joint.angle, (0, 1, 0))
        horiz_ind(30, -0.8 * self.car.hull.angularVelocity, (1, 1, 0))
        vl = pyglet.graphics.vertex_list(
...
if __name__ == "__main__":
    ...
    while isopen:
        ...
        index = 0
        while True:
            if automatic and index<len(default_actions): 
                # a = sa.get()
                a = default_actions[index]
                print(a)
                index += 1

            elif index>=len(default_actions):
                index = 0
                reset = True
